...
```bash
pidprobe snap <PID> [--pretty] [--timeout SECONDS]
```
...
| `stacks` | Per-thread call stacks: file, line, function and a bounded repr of every local variable, innermost frame first |
